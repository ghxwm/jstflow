...
//-------------------------------- Constants ---------------------------------
        ...
        function P(iid, base, offset, val) {
            ...
            var base_c = getConcrete(base);//concrete base
            				 var offset_c = getConcrete(offset);
            if(base_c == null){
            	debugger;
            }
        ...
        function Ra() {
        	...
        	//进入脚本
     function Se(iid,val,isEval) {
            scriptCount++;
            sandbox.analysis.scriptEnter(iid,val,isEval);
        }
     
     function getScriptCount(){return scriptCount;}
     	//退出脚本
        function Sr(iid) {
            scriptCount--;
   ...
   function R(iid, name, val, isGlobal) {
	   					var SPECIAL_PROP_WITH = "*with*",wo;
	   					if(val && HOP(val,SPECIAL_PROP_WITH)){
	   						wo=val[SPECIAL_PROP_WITH];
	   					}
	   					sandbox.analysis.readPre(iid, name, val, isGlobal);
            		
            val = sandbox.analysis.read(iid, name, val, isGlobal);
            if(wo){
            					val[SPECIAL_PROP_WITH]=wo;
            				}
	...
	function Wo(iid,obj){return sandbox.analysis.withObject(iid,obj);}
	...
	sandbox.Ce = Ce;
        ...
        sandbox.endExecution = endExecution;//all script exit
        			sandbox.beginExecution = beginExecution;
        			sandbox.getScriptCount=getScriptCount;
        sandbox.log = log;
})(T$)
