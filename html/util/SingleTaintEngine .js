...
(function(sandbox){
   ...
   var SPECIAL_PROP_OBJ = "*T$*";   //对象字面量标记
		...
		var TFC=0,ZFC=0,VCC=0,VIC=0,SC=0;//taint flow count,violation flow count;
       ...
       /*this.hasLocation = function(objName,propName){
        ...
        this.labelForUrl = function (){
        	...
        	function isSameOriginal(u1,u2){
        			...
        			var pageUrl = HOST_URL,curl=this[SPECIAL_PROP_URL],scriptUrl = new Uri(curl),label ;
    						if(curl == '@sp' || isSamePage(pageUrl,scriptUrl)){
    							label= SLABEL.UMARK.SAME_PAGE;
    						}else if(curl == '@sd' || isSameDomain(pageUrl,scriptUrl)){
    							label = SLABEL.UMARK.SAME_DOMAIN;
    						}else if(curl == '@so' || isSameOriginal(pageUrl,scriptUrl)){
       ...
       this.checkRead = function(base,offset,val){//检查是否可以将val赋值给base,offset.
       		...
       		function incCounter(counter,path){
       						var l = c = counter,p,i;	//l is last path element,c is current path element
       						for(i in path){
       							p = path[i];
       							l = c;
       							c[p] = c[p] || {};
       ...
       function isDom(val){
        ...
        this.getField = function(iid, base, offset, val,ignoreInject) {
        			var base_c = getConcrete(base),offset_c = getConcrete(offset);
        			//处理string,array的长度属性
        ...
        this.readPre = function(iid, name, val, isGlobal) {  //read check
        					...
        					if(HOP(val,SPECIAL_PROP_WITH)){
        							this.injectTaint(val[SPECIAL_PROP_WITH],name,val);
        					}else if(isGlobal){
        ...
        this.conditional = function (iid, left, result_c) {
            return left;
        		...
        		this.withObject = function(iid,obj){
        			var obj_c = getConcrete(obj),ret={},k;
        						if(typeof obj_c == 'object'){
        							for(k  in obj_c){
        								if(HOP(obj_c,k)){
        													ret[k]=this.getField(iid, obj, k, obj_c[k],true);
        ...
        this.endExecution = function() {
        		...
        		function createIndicator(self){
        			var h = document.createElement("div");
        						h.style.position='absolute';
        								h.style.right='5px';
        									h.style.top='5px';
        									h.innerHTML='<a href="javascript:void;">show taint</a>';
        									h.addEventListener('click',showTaintStatical);
        			document.body.appendChild(h);
        		}
        
        		 function showTaintStatical(){
        					var id = "taint_statical";
        					...
        					e.innerHTML=createTaintStaticHtml();
        		}
        		
        		function createTaintStaticHtml(){
        			var h = "<div>";
        						h+="<div>taint flow ratio:"+TFC+"/"+ZFC+"</div>";
        									h+="<div>script count:"+SC+"</div>";
        											h+="confidential violation:"+VCC+"<br>";
        														h+="integrity violation:"+VIC+"<br>";
        														var objName,propName,url,cnt,it;
        							        					h+="object\tproperty\turl\tread count<br>";
        							        					for(objName in TRC){
        							        							it_obj = TRC[objName];
        							        							for(propName in it_obj){
        							        										it_prop = it_obj[propName];
        							        										for(url in it_prop){
        							        														cnt = it_prop[url];
        							        														h+="{0}\t{1}\t{2}\t{3}".format(objName,propName,url,cnt)+"<br>";
        							        													}
        							        										}
        							        							}
        							        				h+="\nobject\tproperty\tlabel\twrite count<br>";
        							        				for(objName in TWC){
        							    								it_obj = TWC[objName];
        							    								for(propName in it_obj){
        							    										it_prop = it_obj[propName];
        							    										for(label in it_prop){
        							    														cnt = it_prop[label];
        							    														h+="{0}\t{1}\t{2}\t{3}".format(objName,propName,label,cnt)+"<br>";
        							    													}
        							    										}
        							    							}
        				h+="</div>";
        ...
        this.scriptEnter = function(iid,url,isEval){
        					if(!isEval)SC++;
        					//log = htmlLog;
